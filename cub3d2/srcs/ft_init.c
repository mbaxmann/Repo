...
/*   Updated: 2020/08/30 20:33:44 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (!ft_check_data(data) && get_next_line(fd, &line))
	{
		if (ft_strncmp(line, "", 2))
		{
			...
			if (line[0] == 'S' && line[1] == 'O')
				data->texture[1] = ft_load_t(data, ft_strtrim(line, " SO"), mlx_ptr);
			else if (line[0] == 'N')
				data->texture[0] = ft_load_t(data, ft_strtrim(line, " NO"), mlx_ptr);
			else if (line[0] == 'W')
				data->texture[2] = ft_load_t(data, ft_strtrim(line, " WE"), mlx_ptr);
			else if (line[0] == 'E')
				data->texture[3] = ft_load_t(data, ft_strtrim(line, " EA"), mlx_ptr);
			else if (line[0] == 'S')
				data->texture[4] = ft_load_t(data, ft_strtrim(line, " S"), mlx_ptr);
			else if (line[0] == 'F')
				data->floor = ft_get_rgb(data, ft_strtrim(line, " F"));
			else if (line[0] == 'C')
				data->ceil = ft_get_rgb(data, ft_strtrim(line, " C"));
			else if (line[0] == 'R')
