...
/*   Updated: 2020/09/01 12:04:37 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_spritesize(t_data *data, t_vector *pt)
{
	...
	size = (int)floor(((double)CUBE *
	((data->res->x / 2) / tan(M_PI / 6))) / pt->dir_x);
	size_max = (int)floor(((double)CUBE *
	((data->res->x / 2) / tan(M_PI / 6))) / (CUBE / 2));
	if (size > size_max)
		return (size_max);
	return (size);
}
